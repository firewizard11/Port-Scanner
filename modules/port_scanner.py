...
class PortScanner:
    ...
    def sequential_scan(self) -> list[int]:
        ...
        """
        if self.verbose:
            print(f"[*] Starting Sequential Scan on {self.target_host}")
        open_ports = []
    ...
    def concurrent_scan(self) -> list[int]:
        ...
        if self.verbose:
            print(
                f"[*] Starting Concurrent Scan on {self.target_host} (max_probes={self.max_probes})"
            )
        open_ports = []
        ...
        with ThreadPoolExecutor(self.max_probes) as pool:
            try:
                futures = {
                    pool.submit(self.tcp_probe, port): port
                    for port in self.target_ports
                }
